...
import model._
import scala.tools.nsc.doc.base.comment.{Block, Body, Comment, Inline}
  ...
  def createEntity(e: Entity): JObject = {
    ...
    as[MemberEntity](e) { m =>
      ...
      if(compactFlags) {
        ...
        if(m.isVar) set(j, 'V')
        if(m.isImplicitlyInherited) set(j, 'm')
        ...
        if(m.isImplicitlyInherited) j += "isImplicit" -> true

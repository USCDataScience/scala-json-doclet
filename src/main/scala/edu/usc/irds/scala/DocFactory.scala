...
class DocFactory (val reporter: Reporter, val settings: doc.Settings) { processor =>
  ...
  def document(files: List[String]): Unit = {
    ...
    if (!reporter.hasErrors) {
      val modelFactory = new model.ModelFactory(compiler, settings) with ModelFactoryImplicitSupport
      ...
      val docModel = modelFactory.makeModel getOrElse  {throw new RuntimeException("Invalid State")}
      println("model contains " + modelFactory.templatesCount + " documentable templates")
      settings.docformat.value match {
        case "json" => new json.JsonFactory(docModel) generate
	    }
    }
  }
